...
<<<<<<< HEAD
import bcrypt from 'bcrypt'     // npm install bcrypt 비밀번호 암호화
=======
import bcrypt from 'bcrypt';        // npm install bcrypt (암복호화 라이브러리)
>>>>>>> 9580dfbccf652b3aaa8dd4138e1a706db183bfd6
...
<<<<<<< HEAD
                // 만약 찾았으면 그 유저 정보를 return

                let db = (await connectDB).db('mydb');
                await  db.collection('user').findOne({emaill:credentials.email});
                ...
                // 비밀번호를 bcrypt로 암호화 했기 때문에 복호화 해서 비교
